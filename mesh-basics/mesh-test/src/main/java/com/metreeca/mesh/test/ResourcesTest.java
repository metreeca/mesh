...
final class ResourcesTest {
    ...
    @Test void testOffices() {
        ...
        assertThat(array(Resources.Offices.stream().map(Record::value).toList()).validate()).isEmpty();
    }

    @Test void testEmployees() {
        assertThat(Resources.Employees.size()).isEqualTo(23);
        assertThat(array(Resources.Employees.stream().map(Record::value).toList()).validate()).isEmpty();
    }
